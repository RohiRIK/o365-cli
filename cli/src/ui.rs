...
pub fn render(f: &mut Frame, app: &mut App) {
    ...
    if let Some(output) = &app.task_output {
        ...
        let (prompt_title, prompt_text) = match &app.input_context {
            crate::app::InputContext::OffboardUserEmail => ("Graceful Offboarding (1/3)", "Enter User Email (UPN):"),
            crate::app::InputContext::OffboardManagerEmail { .. } => ("Graceful Offboarding (2/3)", "Enter Manager Email (Optional):"),
            crate::app::InputContext::OffboardDeviceAction { .. } => ("Graceful Offboarding (3/3)", "Device Action (none/retire/wipe):"),
            crate::app::InputContext::GuestCleanupThreshold => ("Guest User Cleanup", "Enter Inactivity Threshold (Days):"),
