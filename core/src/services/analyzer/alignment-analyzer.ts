...
import type { Policy, PolicyMatch, PolicyGap } from "./ca-baseline";

export type AlignmentStatus = "Aligned" | "Partially Aligned" | "Not Aligned";
...
export interface ExclusionImpact {
  percentage: number; // 0-100 estimated coverage reduction
  penalty: number; // 0-40 alignment score penalty
  highRisk: boolean; // Flag if critical users/apps excluded
  compensatingControls?: string[]; // Suggested alternatives
}
...
export class AlignmentAnalyzer {
   ...
   */
  public analyzeMatch(match: PolicyMatch, policy: Policy, checkId: string): PolicyMatch {
   ...
   * - <50 pts: "Not Aligned"
   */
  private determineAlignment(match: PolicyMatch, policy: Policy): AlignmentAnalysis {
    let score = 100;
    const issues: string[] = [];
   ...
   * - 30%+: -40 points
   */
  private calculateExclusionImpact(policy: Policy): ExclusionImpact {
    const exclusions = {
   ...
   */
  private generateDetailedGaps(
    match: PolicyMatch,
    policy: Policy,
    alignment: AlignmentAnalysis
  ): PolicyGap[] {
    const gaps: PolicyGap[] = [...(match.gaps || [])];
    ...
    if (alignment.exclusionImpact && alignment.exclusionImpact.percentage > 0) {
      ...
      const message = `Policy excludes ${alignment.exclusionImpact.percentage}% of scope${
        alignment.exclusionImpact.highRisk ? " (includes critical apps/users)" : ""
      }`;
      ...
      gaps.push({
        ...
        message,
        suggestion:
          alignment.exclusionImpact.compensatingControls?.[0] ||
