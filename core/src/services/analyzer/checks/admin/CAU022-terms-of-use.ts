...
export function createCAU022Check(helpers: CheckHelpers): BaselineCheck {
  return {
    id: "CAU022",
    name: "Terms of Use Acceptance",
    description:
      "Requires users to accept AUP/ToU before accessing corporate resources.",
    priority: 2,
  ...
  "grantControls": {
    ...
    evaluate(policies: Policy[]): PolicyMatch[] {
      ...
      for (const policy of policies) {
        ...
        const hasTermsOfUse =
          (policy.grantControls as any)?.termsOfUse &&
          (policy.grantControls as any).termsOfUse.length > 0;
