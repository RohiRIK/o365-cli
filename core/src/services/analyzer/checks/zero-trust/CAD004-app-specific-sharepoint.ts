...
export function createCAD004Check(helpers: CheckHelpers): BaselineCheck {
  return {
    id: "CAD004",
    name: "Require Compliant Device for SharePoint/OneDrive",
    description:
      "Enforces device compliance specifically for file storage apps with sensitive data.",
    priority: 3,
    pillar: "Zero Trust",
    recommendation:
      "Create a dedicated policy requiring compliant devices for SharePoint and OneDrive access.",
  ...
  "sessionControls": {
    ...
    evaluate(policies: Policy[]): PolicyMatch[] {
      ...
      for (const policy of policies) {
        ...
        const requiresCompliance = helpers.meetsGrantControls(
          policy,
