...
export function createCAD003Check(helpers: CheckHelpers): BaselineCheck {
  return {
    id: "CAD003",
    name: "Enable Continuous Access Evaluation (CAE)",
    description:
      "Enables real-time session revocation on password change, user deletion, or IP change.",
    priority: 3,
    pillar: "Zero Trust",
    recommendation:
      "Enable CAE for all users and CAE-capable applications (Office 365, Teams, SharePoint).",
    ...
    evaluate(policies: Policy[]): PolicyMatch[] {
      ...
      for (const policy of policies) {
        ...
        const hasSessionControls =
          policy.sessionControls?.signInFrequency !== null ||
