...
export function createCAD007Check(helpers: CheckHelpers): BaselineCheck {
  return {
    id: "CAD007",
    name: "Block Download on Unmanaged Devices (SharePoint/OneDrive)",
    description:
      "Allows web-only SharePoint/OneDrive access but blocks download/sync on unmanaged devices.",
    priority: 3,
    pillar: "Zero Trust",
    recommendation:
      "Enable application-enforced restrictions for SharePoint on unmanaged devices.",
    ...
    evaluate(policies: Policy[]): PolicyMatch[] {
      ...
      for (const policy of policies) {
        ...
        const hasAppRestrictions =
          (policy.sessionControls as any)?.applicationEnforcedRestrictions?.isEnabled === true;
