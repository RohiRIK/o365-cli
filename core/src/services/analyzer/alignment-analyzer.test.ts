...
import type { Policy, PolicyMatch } from "./ca-baseline";
...
describe("AlignmentAnalyzer", () => {
  ...
  const createPolicyMatch = (
    overrides: Partial<PolicyMatch> = {}
  ): PolicyMatch => {
    return {
  ...
  describe("Compensating Controls Suggestions", () => {
    it("should suggest network restrictions for excluded users", () => {
      ...
      expect(enhanced.exclusionImpact?.compensatingControls).toContain(
        "Implement network location restrictions for excluded users"
      );
      expect(enhanced.exclusionImpact?.compensatingControls).toContain(
        "Require device compliance for excluded accounts"
      );
    });
    ...
    it("should suggest dedicated policy for excluded apps", () => {
      ...
      expect(enhanced.exclusionImpact?.compensatingControls).toContain(
        "Create dedicated policy for excluded applications"
      );
    });
    ...
    it("should suggest stronger controls for excluded roles", () => {
      ...
      expect(enhanced.exclusionImpact?.compensatingControls).toContain(
        "Implement stronger controls for excluded roles"
      );
    });
  });
  ...
  describe("Gap Generation", () => {
    ...
    it("should add scope gap for partial user scope", () => {
      ...
      const scopeGap = enhanced.gaps?.find(
        (g) => g.type === "scope" && g.message.includes("partial user scope")
      );
