...
export class GraphService {
     ...
     */
    static async get(endpoint, version = 'v1.0') {
     ...
     */
    static async fetchAll(endpoint, selectOrOptions, version = 'v1.0') {
        const client = this.getClient();
        let results = [];
        ...
        if (typeof selectOrOptions === 'string') {
            ...
            if (selectOrOptions.select)
                request = request.select(selectOrOptions.select);
            if (selectOrOptions.filter)
                request = request.filter(selectOrOptions.filter);
            if (selectOrOptions.expand)
                request = request.expand(selectOrOptions.expand);
            if (selectOrOptions.top)
                request = request.top(selectOrOptions.top);
        }
