 ...
 * - No mailbox activity
 */
export async function detectStaleAccounts(days = 180, dryRun = true) {
    ...
    try {
        ...
        const users = await GraphService.fetchAll(`/users`, "id,userPrincipalName,displayName,accountEnabled,createdDateTime,signInActivity,userType");
        ...
        for (const user of users) {
            if (!user.accountEnabled)
                continue; // Skip already disabled accounts
            if (user.userType === "Guest")
                continue; // Skip guest accounts (handled by iam:guest-cleanup)
