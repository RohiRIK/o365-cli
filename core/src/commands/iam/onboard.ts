...
export async function onboardUser(
  ...
  try {
    ...
    const defaultDomain =
      organization.value && organization.value.length > 0
        ? organization.value[0].verifiedDomains?.find((d: any) => d.isDefault)?.name
        : null;
    ...
    if (templateUser) {
      ...
      try {
        ...
        if (templateLicenses.assignedLicenses && templateLicenses.assignedLicenses.length > 0) {
          const licenseSkuIds = templateLicenses.assignedLicenses.map((l: any) => l.skuId);

          try {
            await client.api(`/users/${userId}/assignLicense`).post({
              addLicenses: licenseSkuIds.map((skuId: string) => ({
                skuId,
                disabledPlans: [],
              })),
              removeLicenses: [],
