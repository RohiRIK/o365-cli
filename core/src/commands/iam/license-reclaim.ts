 ...
 */
export async function reclaimLicenses(dryRun: boolean = true) {
  ...
  try {
    ...
    const users = await GraphService.fetchAll(
      `/users`,
      "id,userPrincipalName,displayName,accountEnabled,assignedLicenses,signInActivity"
    );
    ...
    const premiumSkus: { [key: string]: string } = {
    ...
    candidates.sort((a, b) => {
      if (a.reason.includes("disabled") && !b.reason.includes("disabled")) return -1;
      if (!a.reason.includes("disabled") && b.reason.includes("disabled")) return 1;
