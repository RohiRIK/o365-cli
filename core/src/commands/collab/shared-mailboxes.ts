...
export async function auditSharedMailboxes() {
  ...
  try {
    ...
    const issues: SharedMailboxIssue[] = [];
    ...
    for (const user of usersToCheck) {
      try {
        ...
        const looksLikeSharedMailbox =
          user.userPrincipalName.toLowerCase().includes("shared") ||
    ...
    if (oversized.length > 0) {
      IPC.log(
        `2. ${oversized.length} oversized shared mailboxes - archive or convert to M365 groups`,
        "warn"
      );
    }
    if (highDelegates.length > 0) {
      IPC.log(
        `3. ${highDelegates.length} mailboxes with >5 delegates - consider M365 group for better collaboration`,
        "warn"
      );
    }
