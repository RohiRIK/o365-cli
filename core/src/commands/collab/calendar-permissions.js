...
export async function auditCalendarPermissions() {
    ...
    try {
            ...
            try {
                ...
                if (permissions && permissions.value && permissions.value.length > 0) {
                    for (const perm of permissions.value) {
                        ...
                        const isExternal = perm.emailAddress &&
        ...
        IPC.log(`${risks.length} calendar permission issues found`, risks.length > 0 ? "warn" : "info");
        ...
        if (critical.length > 0) {
            IPC.log(`1. ${critical.length} critical issues - external users with write/owner access - REVOKE IMMEDIATELY`, "error");
        }
        if (high.length > 0) {
            IPC.log(`2. ${high.length} high-risk calendar shares - review external sharing policies`, "warn");
        }
        if (externalShares.length > 0) {
            IPC.log(`3. ${externalShares.length} calendars shared externally - audit and enforce time-limited sharing`, "warn");
        }
        IPC.log("4. Implement calendar sharing policies to prevent oversharing", "info");
        IPC.log("5. Educate users on calendar security best practices (default to 'Free/Busy' for external)", "info");
    }
    catch (error) {
