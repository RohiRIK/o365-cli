 ...
 */
export async function auditExternalSharing(dryRun: boolean = true) {
  ...
  try {
    ...
    try {
      const sites = await GraphService.fetchAll(
        `/sites?search=*`,
        "id,name,webUrl,sharingCapabilities"
      );
      ...
      for (const site of sitesToCheck) {
        ...
        try {
          ...
          if (drive && drive.id) {
            ...
            const permissions = await GraphService.fetchAll(
              `/sites/${site.id}/drive/root/permissions`,
              "id,link,grantedToV2,expirationDateTime"
            );
