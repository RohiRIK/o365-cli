...
function calculateRiskScore(riskyScopes, hasWildcard, hasOffline, publisherVerified, appOwnerType, credHealth, daysSinceLastSignIn, userEnabled, userType, consentType, credentialAgeDays // NEW
) {
    let score = 0;
    // Permission Severity (0-40 points)
    if (hasWildcard)
        score += 20;
    if (riskyScopes.some(s => s.includes("Directory.ReadWrite.All")))
        score += 15;
    if (riskyScopes.some(s => s.includes("Mail.Read") || s.includes("Files.Read.All")))
        score += 10;
    if (hasOffline)
        score += 5;
    ...
    if (!publisherVerified)
        score += 15;
    if (appOwnerType === "ThirdParty")
        score += 10;
    // Credential Hygiene (0-15 points)
    if (credHealth.includes("EXPIRED"))
        score += 10;
    if (credHealth.includes("EXPIRING"))
        score += 5;
    if (credentialAgeDays > 365)
        score += 5; // Stale credential
    // User Context (0-20 points)
    if (daysSinceLastSignIn > 180)
        score += 10; // Zombie grant
    if (!userEnabled)
        score += 5;
    if (userType === "Guest")
        score += 5;
...
let cachedTenantId = "";
async function getTenantId(client) {
    if (cachedTenantId)
...
function classifyPermissions(permissions) {
    try {
        ...
        for (const perm of permissions) {
            if (!perm)
                continue;
            if (PERMISSION_SEVERITY.CRITICAL.includes(perm))
                hasCritical = true;
            else if (PERMISSION_SEVERITY.HIGH.includes(perm))
                hasHigh = true;
            else if (PERMISSION_SEVERITY.MEDIUM.includes(perm))
                hasMedium = true;
        }
...
function generateRecommendation(grant) {
...
function resolveScopeDescriptions(scopes) {
    const scopeMap = {
...
export async function analyzeShadowIT(dryRun = true) {
    ...
    try {
        ...
        const allServicePrincipals = {
            value: allServicePrincipalsRaw.value.filter((sp) => sp.appId &&
        ...
        const managerCache = new Map(); // UserId -> ManagerEmail
        const riskyGrants = [];
        const now = new Date();
        ...
        for (const grant of grants) {
            ...
            if (app.appOwnerOrganizationId === MICROSOFT_FIRST_PARTY_ORG_ID)
                continue;
            // Skip Whitelisted Apps
            if (WHITELISTED_APP_IDS.includes(app.appId))
                continue;
            ...
            const appOwnerType = app.appOwnerOrganizationId === MICROSOFT_FIRST_PARTY_ORG_ID ? "Microsoft" :
                app.appOwnerOrganizationId === await getTenantId(client) ? "Internal" :
                    "ThirdParty";
            const publisherVerified = !!app.verifiedPublisher?.displayName;
            // Calculate risk score
            const riskScore = calculateRiskScore(riskyScopeList, hasWildcardPermissions, hasOfflineAccess, publisherVerified, appOwnerType, credHealth, daysSinceLastSignIn, user.accountEnabled, user.userType, grant.consentType, credentialAgeDays);
            // Classify permissions and generate recommendation
            let permissionSeverity = "LOW";
            ...
            try {
                ...
                for (const assignment of appRoleAssignments.value) {
                    ...
                    const resourceSP = await client.api(`/servicePrincipals/${assignment.resourceId}`)
                        .select("appRoles")
                        .get();
                    const appRole = resourceSP.appRoles?.find((role) => role.id === assignment.appRoleId);
                    ...
                    const appOwnerType = app.appOwnerOrganizationId === MICROSOFT_FIRST_PARTY_ORG_ID ? "Microsoft" :
                        app.appOwnerOrganizationId === await getTenantId(client) ? "Internal" :
                            "ThirdParty";
                    const riskScore = calculateRiskScore([permissionValue], hasWildcard, false, publisherVerified, appOwnerType, credHealth, 0, // No user context for app permissions
                    true, "N/A", "Admin", credentialAgeDays);
                    // Classify permissions and generate recommendation
                    let permissionSeverity = "LOW";
                    let recommendation = "Monitor for unusual activity";
                    let classificationSource = "Internal High-Risk List";
        ...
        try {
            ...
            tableRows = displayGrants.map((g, idx) => {
                try {
                    if (idx < 5)
                        console.error(`[DEBUG] Formatting row ${idx + 1}/${displayGrants.length}`);
                    ...
                    // Severity badge
                    const severityEmoji = g.permissionSeverity === "CRITICAL" ? "🚨" :
