 ...
 */
export async function detectRiskySignIns(days: number = 7, dryRun: boolean = true) {
  ...
  try {
    ...
    const riskySignIns = await GraphService.fetchAll(
      `/auditLogs/signIns`,
      "id,createdDateTime,userPrincipalName,appDisplayName,ipAddress,location,status,riskLevel,riskDetail,riskState",
      {
    ...
    if (riskySignIns.length === 0) {
      ...
      const sorted = riskySignIns.sort((a, b) => {
        const riskOrder: { [key: string]: number } = { high: 0, medium: 1, low: 2, none: 3 };
