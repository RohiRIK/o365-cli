 ...
 */
export async function auditMailboxPermissions(dryRun = true) {
    ...
    try {
        ...
        const users = await GraphService.fetchAll(`/users`, "id,userPrincipalName,displayName,department,signInActivity");
                                ...
                                if (delegateEmail) {
                                    ...
                                    if (delegate) {
                                        ...
                                        if (delegate.signInActivity?.lastSignInDateTime) {
                                            const daysSinceSignIn = Math.floor((Date.now() - new Date(delegate.signInActivity.lastSignInDateTime).getTime()) /
                                                (1000 * 60 * 60 * 24));
        ...
        try {
            const sharedMailboxes = users.filter(u => u.userPrincipalName && u.userPrincipalName.toLowerCase().includes("shared"));
