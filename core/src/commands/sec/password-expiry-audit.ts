 ...
 */
export async function auditPasswordExpiry(days: number = 30, dryRun: boolean = true) {
  ...
  try {
    ...
    const users = await GraphService.fetchAll(
      `/users`,
      "id,userPrincipalName,displayName,passwordPolicies"
    );
    ...
    const neverExpire = users.filter(user =>
