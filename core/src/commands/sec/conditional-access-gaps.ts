 ...
 */
export async function analyzeConditionalAccessGaps(dryRun: boolean = true) {
  ...
  try {
    ...
    const policies = await GraphService.fetchAll(
      ...
      "id,displayName,state,conditions,grantControls,sessionControls"
    );
    ...
    const adminPolicies = enabledPolicies.filter(p =>
      JSON.stringify(p.conditions?.users).includes("Administrator")
    );
    ...
    const legacyAuthPolicies = enabledPolicies.filter(p =>
