 ...
 */
export async function auditPrivilegedAccess(dryRun: boolean = true) {
  ...
  try {
    ...
    const roles = await GraphService.fetchAll(
      `/directoryRoles`,
    ...
    const privilegedUsers: Map<string, PrivilegedUser> = new Map();

    // Fetch role members for each role
    for (const role of roles) {
      try {
        const members = await GraphService.fetchAll(
