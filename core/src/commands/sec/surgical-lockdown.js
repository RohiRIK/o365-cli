 ...
 */
export async function executeSurgicalLockdown(userEmail) {
    ...
    try {
        ...
        const users = await GraphService.fetchAll(`/users?$filter=userPrincipalName eq '${userEmail}'`, "id,userPrincipalName,displayName,accountEnabled");
        if (users.length === 0) {
        ...
        try {
            const devices = await GraphService.fetchAll(`/users/${userId}/managedDevices`, "id,deviceName,operatingSystem");
