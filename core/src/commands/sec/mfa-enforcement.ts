...
interface MFAAuditStats {
  totalUsers: number;
  usersWithMFA: number;
  usersWithoutMFA: number;
  adminWithoutMFA: number;
 ...
 */
export async function auditMFAEnforcement(dryRun: boolean = true) {
  ...
  try {
    ...
    const users = await GraphService.fetchAll(
      `/users`,
      "id,userPrincipalName,displayName,accountEnabled,assignedLicenses",
      {
    ...
    const userMFAStatus: UserMFAStatus[] = [];
    ...
    for (const user of users) {
      try {
        ...
        const authMethods = await GraphService.fetchAll(
        ...
        authMethods.forEach((method: any) => {
    ...
    try {
      const policies = await GraphService.fetchAll(
        `/identity/conditionalAccess/policies`,
        "id,displayName,state,grantControls,conditions",
        {}
      );

      IPC.log(`Found ${policies.length} Conditional Access policies`, "info");

      conditionalAccessPolicies = policies.map((p: any) => {
        const requiresMFA =
    ...
    try {
      ...
      const signIns = await GraphService.fetchAll(
        `/auditLogs/signIns`,
        "id,createdDateTime,userPrincipalName,clientAppUsed,authenticationRequirement",
        {
