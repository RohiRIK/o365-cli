 ...
 */
export async function auditAppPermissions(dryRun: boolean = true) {
  ...
  try {
    ...
    const apps = await GraphService.fetchAll(
      `/servicePrincipals`,
      "id,appDisplayName,appId,servicePrincipalType"
    );
    ...
    const riskyApps = apps.filter(app =>
      app.servicePrincipalType === "Application" &&
      app.appDisplayName &&
