 ...
 */
export async function analyzeUser(userEmail) {
    ...
    try {
        ...
        try {
            user = await client
                .api(`/users/${userEmail}`)
                .select("id,userPrincipalName,displayName,jobTitle,department,accountEnabled,createdDateTime,assignedLicenses")
                .get();
        }
        catch (error) {
        ...
        try {
            ...
            for (const site of sites.value || []) {
                try {
                    ...
                    const isOwner = siteOwners.value?.some((owner) => owner.userPrincipalName === userEmail);
                    if (isOwner) {
        ...
        if (groupCount > 0) {
            IPC.success({
                ...
                table: {
                    ...
                    rows: groups.value.slice(0, 10).map((g) => [
        ...
        if (deviceCount > 0) {
            IPC.success({
                ...
                table: {
                    ...
                    rows: devices.value.slice(0, 10).map((d) => [
        ...
        if (oneDriveFiles.length > 0) {
            IPC.success({
                ...
                table: {
                    ...
                    rows: oneDriveFiles.slice(0, 10).map((f) => [
        ...
        if (mailRules.length > 0) {
            IPC.success({
                ...
                table: {
                    ...
                    rows: mailRules.slice(0, 10).map((r) => [
        ...
        if (calendarDelegates.length > 0) {
            IPC.success({
                ...
                table: {
                    ...
                    rows: calendarDelegates.slice(0, 10).map((d) => [
