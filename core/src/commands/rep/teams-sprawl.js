 ...
 */
export async function analyzeTeamsSprawl(inactivityDays = 90) {
    ...
    try {
        ...
        const teams = await GraphService.fetchAll(`/groups?$filter=resourceProvisioningOptions/Any(x:x eq 'Team')`, "id,displayName,mail,createdDateTime,renewedDateTime,memberCount");
        ...
        IPC.log(`${inactiveTeams.length} inactive Teams (>${inactivityDays} days)`, inactiveTeams.length > 0 ? "warn" : "info");
        // Sort by severity (High > Medium > Low) and then by days inactive
        const severityOrder = { High: 0, Medium: 1, Low: 2 };
        inactiveTeams.sort((a, b) => (severityOrder[a.severity] || 999) - (severityOrder[b.severity] || 999) ||
        ...
        if (largeStorage.length > 0) {
            IPC.log(`3. ${largeStorage.length} teams consuming >5GB - prioritize for storage reclamation`, "warn");
        }
