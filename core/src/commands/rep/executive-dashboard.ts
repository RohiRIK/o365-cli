 ...
 * - Storage usage
 */
export async function generateExecutiveDashboard(dryRun: boolean = true) {
  ...
  try {
    ...
    const users = await GraphService.fetchAll(
      `/users`,
      "id,userPrincipalName,accountEnabled,assignedLicenses"
    );
    ...
    const devices = await GraphService.fetchAll(
      ...
      "id,operatingSystem,complianceState"
    );
