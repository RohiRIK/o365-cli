 ...
 */
export async function analyzeMailboxSizing(thresholdGb: number = 50, dryRun: boolean = true) {
  ...
  try {
    ...
    const users = await GraphService.fetchAll(
      `/users`,
      "id,userPrincipalName,displayName,assignedLicenses,mail"
    );
    ...
    const analyses: MailboxAnalysis[] = [];
