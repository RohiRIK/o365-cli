 ...
 */
export async function auditPowerPlatformLicenses(dryRun = true) {
    ...
    try {
        ...
        const subscribedSkus = await GraphService.fetchAll(`/subscribedSkus`, "id,skuPartNumber,consumedUnits,prepaidUnits");
        // Filter for Power Platform licenses
        const powerPlatformLicenses = subscribedSkus.filter(sku => sku.skuPartNumber in powerPlatformSkus);
        ...
        const users = await GraphService.fetchAll(`/users`, "id,userPrincipalName,displayName,accountEnabled,signInActivity,assignedLicenses");
