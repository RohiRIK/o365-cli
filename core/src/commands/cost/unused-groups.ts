 ...
 */
export async function cleanupUnusedGroups(inactivityDays: number = 180, dryRun: boolean = true) {
  ...
  try {
    ...
    const groups = await GraphService.fetchAll(
      `/groups`,
      "id,displayName,groupTypes,mail,mailEnabled,securityEnabled,createdDateTime,renewedDateTime,memberCount"
    );
