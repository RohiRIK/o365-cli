...
interface LicenseOptimizationArgs {
  dryRun: boolean;
}
 ...
 */
export async function optimizeLicenses(dryRun: boolean = true) {
  ...
  try {
    ...
    const subscribedSkus = await GraphService.fetchAll(
      `/subscribedSkus`,
      "id,skuPartNumber,consumedUnits,prepaidUnits,appliesTo"
    );
    ...
    const users = await GraphService.fetchAll(
      `/users`,
      "id,userPrincipalName,displayName,accountEnabled,signInActivity,assignedLicenses,createdDateTime"
    );
    ...
    // Phase 3: Calculate license utilization (50-80%)
    interface LicenseAnalysis {
      ...
      disabledUserAssignments: number;
      estimatedMonthlyCost: number;
      potentialSavings: number;
    }

    const licenseMap: { [key: string]: LicenseAnalysis } = {};

    // License pricing (approximate monthly cost per user in USD)
    const licensePricing: { [key: string]: number } = {
    ...
    const wastefulLicenses = allLicenses.filter(
      l => l.totalUnused > 0 || l.inactiveAssignments > 0 || l.disabledUserAssignments > 0
    );
