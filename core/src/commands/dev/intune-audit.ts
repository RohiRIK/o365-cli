...
export async function auditIntuneAssignments(dryRun: boolean = true, exportPath?: string) {
    ...
    async function resolveAssignments(endpoint: string, policyId: string, version: 'v1.0' | 'beta' = 'v1.0', policyType?: string): Promise<{ included: string, excluded: string }> {
        try {
                ...
                try {
                    ...
                    if (intentAssignments.value) {
                        ...
                        for (const a of intentAssignments.value) {
                             if (a.target?.groupId) included.push(`Group: ${a.target.groupId}`);
                             else included.push(a.target?.["@odata.type"]?.split('.').pop() || "Assigned");
                        }
    ...
    try {
        const categories: { name: string, endpoint: string, version: 'v1.0' | 'beta' }[] = [
        ...
        for (const cat of categories) {
            ...
            try {
                const policies = await GraphService.fetchAll<Policy>(cat.endpoint, undefined, cat.version);
