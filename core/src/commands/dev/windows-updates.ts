 ...
 */
export async function auditWindowsUpdates(dryRun: boolean = true) {
  ...
  try {
    ...
    const devices = await GraphService.fetchAll(
      ...
      "id,deviceName,operatingSystem,osVersion,complianceState,lastSyncDateTime,userPrincipalName,model",
      {
    ...
    // Phase 2: Analyze Windows versions and update status (30-60%)
    ...
    const analyzedDevices: DeviceAnalysis[] = [];
    const now = new Date();

    // Known Windows versions and their end-of-service dates
    const knownBuilds: { [key: string]: { name: string; endOfService: Date; isSupported: boolean } } = {
      // Windows 11
