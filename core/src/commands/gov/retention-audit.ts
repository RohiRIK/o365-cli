...
interface RetentionPolicy {
  ...
  locations: string[];
  scope: string;
  mode: "preserve" | "delete" | "preserveAndDelete";
}

interface PolicyConflict {
  resource: string;
  policies: string[];
  issue: "overlap" | "gap" | "conflict";
 ...
 * - Disabled policies
 */
export async function auditRetentionPolicies(dryRun: boolean = true) {
  ...
  try {
    ...
    let retentionLabels: any[] = [];

    try {
      retentionLabels = await GraphService.fetchAll(
    ...
    try {
      ...
      retentionPolicies = policies.map((p: any) => ({
        id: p.id,
        ...
        scope: "All",
        mode: "preserveAndDelete" as const,
      }));
    } catch (error: any) {
    ...
    const locationPolicies = new Map<string, string[]>();
