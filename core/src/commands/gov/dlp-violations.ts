...
interface DLPStats {
  ...
  topFileTypes: { fileType: string; count: number }[];
  topPolicies: { policy: string; count: number }[];
  topSensitiveTypes: { type: string; count: number }[];
 ...
 */
export async function generateDLPViolationReport(days: number, dryRun: boolean = true) {
  ...
  try {
    ...
    try {
      securityAlerts = await GraphService.fetchAll(
        `/security/alerts_v2`,
        "id,createdDateTime,title,severity,category,userStates,fileStates,status",
        {
      ...
      securityAlerts = securityAlerts.filter((alert: any) =>
        alert.category?.toLowerCase().includes("dlp") ||
    ...
    try {
      auditLogs = await GraphService.fetchAll(
        `/auditLogs/directoryAudits`,
        "id,activityDateTime,activityDisplayName,category,result,initiatedBy,targetResources,additionalDetails",
        {
      ...
      auditLogs = auditLogs.filter((log: any) =>
        log.category?.toLowerCase().includes("dlp") ||
...
function extractSensitiveType(title: string): string {
  const lowerTitle = title.toLowerCase();
  if (lowerTitle.includes("credit card")) return "Credit Card Number";
  if (lowerTitle.includes("ssn") || lowerTitle.includes("social security")) return "Social Security Number";
  if (lowerTitle.includes("passport")) return "Passport Number";
  if (lowerTitle.includes("pii")) return "Personally Identifiable Information";
  if (lowerTitle.includes("phi") || lowerTitle.includes("health")) return "Protected Health Information";
  if (lowerTitle.includes("financial")) return "Financial Information";
  return "Sensitive Information";
}

function extractSensitiveTypeFromLog(log: any): string {
  const activityName = (log.activityDisplayName || "").toLowerCase();
  if (activityName.includes("confidential")) return "Confidential";
  if (activityName.includes("secret")) return "Secret";
  if (activityName.includes("internal")) return "Internal";
  if (activityName.includes("personal")) return "Personal Data";
  return "Sensitive Information";
}

function getFileTypeFromActivity(activity: string): string {
  const lowerActivity = activity.toLowerCase();
  if (lowerActivity.includes("excel") || lowerActivity.includes(".xlsx")) return "xlsx";
  if (lowerActivity.includes("word") || lowerActivity.includes(".docx")) return "docx";
  if (lowerActivity.includes("pdf")) return "pdf";
  if (lowerActivity.includes("email")) return "email";
  return "unknown";
}

function extractLocation(additionalDetails: any[]): string {
  if (!additionalDetails || additionalDetails.length === 0) return "Unknown";
  const locationDetail = additionalDetails.find((d: any) => d.key?.toLowerCase().includes("location"));
...
function generateStatistics(violations: DLPViolation[]): DLPStats {
  const userCounts = new Map<string, number>();
  const fileTypeCounts = new Map<string, number>();
  const policyCounts = new Map<string, number>();
  const sensitiveTypeCounts = new Map<string, number>();
