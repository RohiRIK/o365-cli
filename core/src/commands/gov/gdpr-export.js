 ...
 */
export async function exportGDPRData(user, dryRun = true) {
    ...
    try {
        ...
        try {
            const groups = await GraphService.fetchAll(`/users/${user}/memberOf`, "displayName,id,groupTypes,mail,securityEnabled,mailEnabled,createdDateTime");
            userData.groups = groups.map((g) => ({
                id: g.id,
        ...
        try {
            const teams = await GraphService.fetchAll(`/users/${user}/joinedTeams`, "id,displayName,description,isArchived,createdDateTime");
            userData.teams = teams.map((t) => ({
                id: t.id,
        ...
        try {
            const devices = await GraphService.fetchAll(`/users/${user}/registeredDevices`, "id,displayName,operatingSystem,operatingSystemVersion,approximateLastSignInDateTime,isCompliant,isManaged");
            userData.devices = devices.map((d) => ({
                id: d.id,
