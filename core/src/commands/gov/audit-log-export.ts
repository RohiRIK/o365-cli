...
export async function exportAuditLogs(
  ...
  try {
    ...
    try {
      ...
      signInLogs = await GraphService.fetchAll(
        `/auditLogs/signIns`,
        "id,createdDateTime,userPrincipalName,appDisplayName,ipAddress,location,status,deviceDetail,riskLevel",
        {
    ...
    let directoryLogs: any[] = [];

    try {
      const startIso = start.toISOString();
      const endIso = end.toISOString();

      directoryLogs = await GraphService.fetchAll(
    ...
    if (operations) {
      ...
      filteredDirectoryAudits = directoryLogs.filter(log =>
        operationList.some(op => log.activityDisplayName?.toLowerCase().includes(op))
      );
    ...
    const allLogs = [
      ...
      ...filteredDirectoryAudits.map(log => ({
        ...
        result: log.result,
        targets: log.targetResources?.map((t: any) => t.displayName).join(", ") || "N/A",
      })),
    ];
      ...
      IPC.success({
        ...
        table: {
          ...
          rows: preview.map(log => [
            ...
            log.user,
            log.activity,
            (log as any).status || (log as any).result || "N/A",
          ]),
        },
      });
