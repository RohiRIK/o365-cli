 ...
 */
function renderScopes(scopes, indent = 4) {
...
async function getSessionDetails(tenantId = "common", includeOrg = true) {
...
async function ensureAuthenticated(tenantId = "common") {
...
async function handleLoginMenu() {
    ...
    try {
        ...
        if (session) {
            ...
            if (session.issuedAt)
                console.log(`  ${chalk.bold("Issued At:")}     ${session.issuedAt.toLocaleString()}`);
            if (session.expires) {
 ...
 */
async function runSearchablePicker(showAll = false) {
    nav.push("Search");
    nav.refresh();
 ...
 */
async function runCategorizedPicker(showAll = false) {
    nav.push("Categories");
    nav.refresh();
 ...
 */
async function interactiveModulePicker(showAll = false) {
    try {
        ...
        if (mode === "exit")
            return "exit";
        if (mode === "search")
            return await runSearchablePicker(showAll);
        return await runCategorizedPicker(showAll);
    }
    catch {
        return "exit";
...
async function showInteractiveMenu(showAll = false) {
    let running = true;
    while (running) {
        nav.clear();
        nav.refresh();
...
async function showStatus() {
    ...
    if (session) {
        ...
        if (session.issuedAt)
            console.log(`  - Issued At:  ${session.issuedAt.toLocaleString()}`);
        if (session.expires) {
            const isExpired = session.expires < new Date();
            const color = isExpired ? chalk.red : chalk.green;
...
async function getOrganizationName(forFilename = false) {
    if (cachedOrgName && !forFilename)
...
async function runTask(moduleName, args) {
    try {
...
export async function setupCLI(program) {
    program
        .version("1.0.0")
